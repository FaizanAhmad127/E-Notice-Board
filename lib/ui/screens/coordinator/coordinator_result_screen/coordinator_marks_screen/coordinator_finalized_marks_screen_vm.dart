...
class CoordinatorFinalizedMarksScreenVM extends ChangeNotifier{
  ...
  CoordinatorFinalizedMarksScreenVM(){
  ...
  {
    try {
      await _resultService.getAllFinanalizedStudentMarks().then((listMarks) async {
  ...
  void getStudentIdea(String? uid) {
    for(var idea in getIdeasList){
      for(var studentID in idea.students){
        print("indea  stdID:"+uid.toString()+"  stdIDfromidea:"+studentID.toString());
        if(studentID == uid){
          print("indea matched");
    ...
    print("indea ideaslist  "+studentIdeasList.length.toString());
  }
}