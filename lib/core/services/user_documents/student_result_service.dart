...
class StudentResultService {
  ...
  Future<List<MarksModel>> getAllFinanalizedStudentMarks() async {
    BotToast.showLoading();
    List<MarksModel> listOfResults = [];
    try {
      await _firebaseFirestore
          ...
          .then((docsSnap) {
        for (var doc in docsSnap.docChanges) {
          var model = MarksModel.fromJson(doc.doc);
          if(model.isfinalized=="yes"){
          listOfResults.add(model);
          }
        }
      });
    } catch (error) {
  ...
  Future<List<MarksModel>> getAllPendingStudentMarks() async {
    BotToast.showLoading();
    List<MarksModel> listOfResults = [];
    try {
      await _firebaseFirestore
 ...
 Future setStudentResultFinalizedMarks(
      String uid) async {
    ...
    try {
        await _firebaseFirestore
            ...
            {"isfinalized": "yes"}
            ,SetOptions(merge: true)).then((value) {
          BotToast.showText(text: 'Result Finalized');
        });
