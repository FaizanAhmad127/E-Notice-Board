...
class CommitteeService {
  ...
  Future setCommitteeDocument(List<String> teacherList,List<String> ideaList, List<String> selectedConvenerList)async
  {
      ...
      await _firebaseFirestore.collection('committee').doc(docId).set(
        {
          ...
          'ideaList':ideaList,
          'convenerList':selectedConvenerList,
        },
    ...
    try {
      await _firebaseFirestore.collection('committee').get().then((querySnap) {
        for (var docSnap in querySnap.docs) {
         ...
         for (var element in committeeModel.teacherList) {
  ...
  Future<List<String>> getListOfAllIdeasInCommittee() async {
    ...
    try {
      await _firebaseFirestore.collection('committee').get().then((querySnap) {
        for (var docSnap in querySnap.docs) {
          CommitteeModel committeeModel= CommitteeModel.fromJson(docSnap);
          for (var element in committeeModel.ideaList) {
  ...
  Future< List<CommitteeModel?>> getTeacherConvener(String teacherUid) async {
    List<CommitteeModel> cm=[];
    try {
      await _firebaseFirestore.collection('committee').get().then((querySnap) {
        for (var docSnap in querySnap.docs) {
          CommitteeModel? committeeModel=CommitteeModel.fromJson(docSnap);
          if(committeeModel.convenerList.contains(teacherUid))
            {
              cm.add(committeeModel);
            }
